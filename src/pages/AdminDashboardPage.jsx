...
export default function AdminDashboardPage() {
    ...
    Papa.parse(csvFile, {
      ...
      complete: async (results) => {
        ...
        try {
          ...
          setUploadStatus({ 
            type: 'success', 
            message: `Berhasil mengimpor ${insertedCount} data ke tabel [${targetTable}]!` 
          });
          setCsvFile(null);
        } catch (err) {
  ...
  if (profile && profile.role?.toLowerCase() !== 'admin' && profile.tipe_akun !== 'KANTOR') {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-100 p-4">
        <div className="bg-white p-8 rounded-2xl shadow-md text-center space-y-3 max-w-md">
          ...
          <p className="text-xs text-slate-500">Halaman ini khusus untuk Pengelola / Admin Sensus.</p>
          <button onClick={logout} className="mt-4 px-4 py-2 bg-slate-900 text-white text-xs font-bold rounded-xl cursor-pointer">Keluar</button>
        </div>
      </div>
    );
  }
